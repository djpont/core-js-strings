...
function formatTime(minutes, seconds) {
  const doubleDigit = (num) => num.toString().padStart(2, '0');
...
function isPalindrome(str) {
  const strWithoutSymbols = str.toLowerCase().replace(/[^a-z]/g, '');
  return strWithoutSymbols === reverseString(strWithoutSymbols);
}
...
function getCardId(value) {
  ...
  const suitsArray = ['♣', '♦', '♥', '♠'];
  const num = value.slice(0, value.length - 1);
  const suit = value.slice(-1);
  return suitsArray.indexOf(suit) * numsArray.length + numsArray.indexOf(num);
}
